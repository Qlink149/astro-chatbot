#!/usr/bin/env python3
"""
Create, upload JSON, and publish a Gupshup WhatsApp Flow for Samara.

Reads .env from backend/ (project root).
...
Optional:
  GUPSHUP_FLOW_NAME          (default: samara_birth_details)
  GUPSHUP_FLOW_CATEGORIES    (default: OTHER) — comma-separated
  GUPSHUP_FLOW_JSON_PATH     (default: json/birth_details.json)
...
def resolve_json_path() -> Path:
    raw = (
        os.environ.get("GUPSHUP_FLOW_JSON_PATH") or "json/birth_details.json"
    ).strip()
    path = Path(raw)
...
def main() -> None:
            ...
            json.dumps(
                {
                    "appId": app_id,
                    "flowName": (
                        os.environ.get("GUPSHUP_FLOW_NAME") or "samara_birth_details"
                    ).strip(),
    ...
    flow_name = (os.environ.get("GUPSHUP_FLOW_NAME") or "samara_birth_details").strip()
    categories = parse_categories()
    ...
    if args.resume:
                    ...
                    json.dumps(
                        {
                            ...
                            "nextSteps": [
                                f"SAMARA_BIRTH_FLOW_ID={published_id}",
        ...
        if not flow_id:
            raise SystemExit(
                ...
                "If already published, set SAMARA_BIRTH_FLOW_ID from --list."
            )
            ...
            json.dumps(
                {
                    ...
                    "flowName": flow_name,
                    "nextSteps": [
                        f"SAMARA_BIRTH_FLOW_ID={flow_id}",
                    ],
                },
                indent=2,
            )
        ...
        json.dumps(
            {
                ...
                "flowName": flow_name,
                "nextSteps": [
                    f"Add to .env and Vercel: SAMARA_BIRTH_FLOW_ID={flow_id}",
