...
def save_to_mongo(data: dict) -> dict | None:
    ...
    client_id = data.get("client_id", "samara")
    try:
        logger.info(
...
def touch_last_message_at(phone_number: str, client_id: str = "samara") -> None:
...
def get_takeover_status(phone_number: str, client_id: str = "samara") -> dict | None:
...
def set_takeover(phone_number: str, active: bool, client_id: str = "samara") -> None:
...
def get_user_by_phone(phone_number: str, client_id: str = "samara") -> dict | None:
...
def search_users(q: str, client_id: str = "samara", limit: int = 20) -> list:
...
def request_live_agent(phone_number: str, client_id: str = "samara") -> None:
...
def save_response_time(
    phone_number: str, response_time_ms: int, client_id: str = "samara"
) -> None:
...
def resolve_live_agent(phone_number: str, client_id: str = "samara") -> None:
...
def get_user_growth(period: str = "month", client_id: str = "samara") -> list:
    """New-user counts grouped by period using created_at."""
    try:
        result = [
            {"period": doc.get("period"), "count": int(doc.get("count", 0))}
...
def get_dashboard_stats(client_id: str = "samara") -> dict:
