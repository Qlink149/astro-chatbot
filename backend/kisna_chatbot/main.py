        ...
        async with lock:
            try:
                ...
                if phone_number_id and phone_number_id not in build_phone_number_id_map():
                    logger.debug(
                        "Webhook phone_number_id not in env map; using DEFAULT_CLIENT_ID "
                        "(optional: set SAMARA_PHONE_NUMBER_ID for explicit routing)",
                        extra={
                            "phone_number_id": phone_number_id,
                            "client_id": client_id,
                        },
            ...
            if message_id:
                ...
                typing_task = asyncio.create_task(
                    typing_indicator_loop(message_id, stop_typing_event)
                )
                ...
                data["bot_response"] = [
                    {
                        "type": "text",
                        "text": (
                            "Maaf kijiye, kuch gadbad ho gayi. "
                            "Please thodi der mein dobara try kijiye. 🙏"
                        ),
                    }
                ]
            await localize_bot_responses(data)
...
async def messages_samara(
    ...
    """Gupshup WhatsApp webhook for Samara."""
